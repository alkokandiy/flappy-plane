# Flappy Plane
A Flappy Bird clone where the bird is a plane and the pipes are skyscraper
towers, with looping background music that plays only during gameplay.

Play in the browser (no build step, works on PC and mobile):
1. `cd web`
2. `python3 -m http.server 8000`
3. Open http://localhost:8000 — click / tap / Space to flap.

Put background music in `web/assets/music/` (`.ogg` works everywhere;
`.m4a` does not — convert it: `ffmpeg -i song.m4a -c:a libvorbis song.ogg`).

Desktop version (Python/pygame, same game logic):
